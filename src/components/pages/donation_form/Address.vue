<template>
	<div class="address-section">
		<form
				name="laika-donation-personal-data"
...
export default Vue.extend( {
	...
	setup( props : any, { root: { $store } } ) {
		const { addressType, isFullSelected } = toRefs( props );
		const addressFunctions = useAddressFunctions( props, $store );

		const showPostal = computed( () => {
			return isFullSelected.value || [ AddressTypeModel.COMPANY, AddressTypeModel.PERSON ].includes( addressType.value );
		} );
		const showEmail = computed( () => {
			return isFullSelected.value || [ AddressTypeModel.EMAIL, AddressTypeModel.COMPANY, AddressTypeModel.PERSON ].includes( addressType.value );
		} );
