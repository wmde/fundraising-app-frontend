<template>
  <button class="content-copier" :class="{ 'content-copier--copied': wasJustCopied }" @click="onCopy" @touchstart="onCopy">
    <span class="content-copier__popup display-toggler" :class="{ 'display-toggler--visible': wasJustCopied }" aria-live="assertive"><template v-if="wasJustCopied">{{ $t( 'copy_popup' ) }}</template></span>
