...
const validateFields = async ( fieldName: 'account-number' | 'bank-code' ): Promise<void> => {
	...
	if ( accountNumberType.value === AccountNumberType.Account && bankCode.value === '' ) {
		return Promise.resolve();
	}
	...
	try {
		if ( accountNumberType.value === AccountNumberType.IBAN ) {
			...
			await Promise.all( [
				store.dispatch( action( 'bankdata', 'setAccountNumber' ), response.iban ),
				store.dispatch( action( 'bankdata', 'setBankCode' ), '' ),
			] );
		} else {
			...
			await Promise.all( [
				store.dispatch( action( 'bankdata', 'setAccountNumber' ), response.accountNumber ),
				store.dispatch( action( 'bankdata', 'setBankCode' ), response.bankCode ),
			] );
		}
		...
		await Promise.all( [
			store.dispatch( action( 'bankdata', 'setBankDataValidity' ), Validity.INVALID ),
			store.dispatch( action( 'bankdata', 'setBankName' ), '' ),
			store.dispatch( action( 'bankdata', 'setIban' ), '' ),
