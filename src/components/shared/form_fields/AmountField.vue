<template>
	<fieldset class="field-container flow" id="payment-form-amount" :class="[ `locale-${ $i18n.locale }` ]" :data-error="showError ? true : null" data-max-width>
		...
		<p class="field-container__help-text">{{ $t( 'donation_form_payment_amount_help_text' ) }}</p>
		<p v-if="minimumAmountMessage!=''" class="field-container__help-text">{{ minimumAmountMessage }}</p>
