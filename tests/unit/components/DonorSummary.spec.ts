...
describe( 'DonorSummary.vue', () => {
	...
	it( 'does not render anything when all address fields are empty', () => {
		const wrapper = mountWrapper( emptyAddress );

		expect( wrapper.find( '.donor-summary' ).exists() ).toBe( false );
	} );
