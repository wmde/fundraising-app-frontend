...
describe( 'TextFormInput.vue', () => {
	...
	describe( 'accessibility tests', () => {
		it( 'sets aria-describedby on input', async () => {
			const wrapper = getWrapper();
		...
		it( 'sets aria-describedby on textarea', async () => {
			const wrapper = getWrapper();

			await wrapper.setProps( { inputType: 'textarea', ariaDescribedby: 'describedby-label' } );

			expect( wrapper.find( 'textarea' ).attributes( 'aria-describedby' ) ).toStrictEqual( 'describedby-label' );
		} );
	} );
} );
