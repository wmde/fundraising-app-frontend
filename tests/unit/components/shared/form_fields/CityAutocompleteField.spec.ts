...
describe( 'CityAutocompleteField.vue', () => {
	...
	describe( 'accessibility tests', () => {
		it( 'sets aria-describedby', async () => {
			...
			await wrapper.setProps( { modelValue: '{"key":"donation_form_city_placeholder"}' } );

			expect( field.attributes( 'aria-describedby' ) ).toStrictEqual( 'city-message' );
		...
		it( 'sets aria-activedescendant', async () => {
			...
			expect( field.attributes( 'aria-activedescendant' ) ).toStrictEqual( 'city-0' );

			await field.trigger( 'keydown', { key: 'ArrowDown' } );

			expect( field.attributes( 'aria-activedescendant' ) ).toStrictEqual( 'city-1' );
		} );
	} );
