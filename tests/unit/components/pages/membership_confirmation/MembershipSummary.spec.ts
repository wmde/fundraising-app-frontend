...
describe( 'MembershipSummary.vue', () => {
	...
	it( 'renders personal membership confirmation data', () => {
		...
		expect( wrapper.find( '.form-summary-content p:last-child' ).text() ).toContain(
	...
	it( 'renders monthly payments', () => {
		...
		expect( wrapper.find( '.form-summary-content p:last-child' ).text() ).toContain( '\"membershipFeeFormatted\": 15' );
		expect( wrapper.find( '.form-summary-content p:last-child' ).text() )
	...
	it( 'renders quarterly payments', () => {
		...
		expect( wrapper.find( '.form-summary-content p:last-child' ).text() ).toContain( '\"membershipFeeFormatted\": 45' );
		expect( wrapper.find( '.form-summary-content p:last-child' ).text() )
			.toContain( '\"membershipFeeYearlyFormatted\": \"(180 donation_form_payment_interval_12)\"' );
	} );
	...
	it( 'renders yearly payments', () => {
		...
		expect( wrapper.find( '.form-summary-content p:last-child' ).text() ).toContain( '\"membershipFeeFormatted\": 180' );
		expect( wrapper.find( '.form-summary-content p:last-child' ).text() ).toContain( '\"membershipFeeYearlyFormatted\": \"\"' );
	} );
