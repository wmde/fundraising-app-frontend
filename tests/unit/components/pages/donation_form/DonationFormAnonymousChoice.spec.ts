...
import DonationForm from '@src/components/pages/donation_form/SubPages/DonationFormAnonymousChoice.vue';
...
describe( 'DonationFormAnonymousChoice.vue', () => {
	...
	it( 'handles the error summary when no address choice was selected before submitting', async () => {
		const wrapper = getWrapper();
		...
		expect( errorSummaryItemIsFunctional( wrapper, 'addressOptOut-1', 'address-opt-out-scroll-target' ) ).toBeTruthy();

		// Select "No" for address question
		await wrapper.find( 'input[name="address-opt-out"][value="false"]' ).trigger( 'change' );
		await nextTick();
		await nextTick();
	...
	it( 'handles the error summary when person address type was selected before submitting', async () => {
		...
		await wrapper.find( 'input[name="address-opt-out"][value="true"]' ).trigger( 'change' );
	...
	it( 'preserves previous address type choice when deciding against an address and then for giving an address', async () => {
		const wrapper = getWrapper();

		await wrapper.find( 'input[name="address-opt-out"][value="true"]' ).trigger( 'change' );

		await wrapper.find( 'input[name="addressType"][value="0"]' ).trigger( 'change' );
		expect( wrapper.find<HTMLInputElement>( 'input[name="addressType"][value="0"]' ).element.checked ).toBeTruthy();

		// Set to company, then destroy form by selecting "no" in the address choice
		await wrapper.find( 'input[name="addressType"][value="1"]' ).trigger( 'change' );
		await wrapper.find( 'input[name="address-opt-out"][value="false"]' ).trigger( 'change' );
		expect( wrapper.find<HTMLInputElement>( 'input[name="addressType"][value="1"]' ).exists() ).toBeFalsy();

		// Show the form again by selecting "yes" in the address choice, check for company
		await wrapper.find( 'input[name="address-opt-out"][value="true"]' ).trigger( 'change' );
		expect( wrapper.find<HTMLInputElement>( 'input[name="addressType"][value="1"]' ).element.checked ).toBeTruthy();
	} );
